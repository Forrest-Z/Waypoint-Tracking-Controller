...
void timerCallback(const ros::TimerEvent& e) 
{
	...
	vel.push_back(v);
	std::cout << "\n X-coord: " << actualPose[0] << "\t Y-coord: " << actualPose[1] ;
	std::cout << "\t Velocity -- " << v*3.6 << "\t Length of velocity : " << vel.size();
...
int main(int argc, char **argv) 
{
	...
	n.param<int>("/controller/min_dist",min_dist,1);
    ...
    n.param<double>("/controller/sample_time",sample_time,1);
	...
	ros::Subscriber spline_sub = n.subscribe("/ThorPlanner/GlobalPath", 1000, trajectoryCallback); 
	...
	timer= n.createTimer(ros::Duration(0.025), timerCallback);
